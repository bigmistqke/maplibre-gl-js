...
// Modifications: import paths, type narrowing, STUB comments for globe
...
/**
 * Narrow interface for the painter object used in this module.
 * Only transform, width, and height are accessed.
 */
interface PainterLike {
    transform: ISymbolTransform;
    width: number;
    height: number;
}

/**
 * Narrow interface for the symbol bucket used in this module.
 */
interface SymbolBucketLike {
    text: {
        dynamicLayoutVertexArray: StructArray<string>;
        dynamicLayoutVertexBuffer?: { updateData(arr: StructArray<string>): void };
        placedSymbolArray: StructArray<string>;
    };
    icon: {
        dynamicLayoutVertexArray: StructArray<string>;
        dynamicLayoutVertexBuffer?: { updateData(arr: StructArray<string>): void };
    ...
    lineVertexArray: StructArray<string>;
    glyphOffsetArray: StructArray<string>;
    textSizeData: any;
    iconSizeData: any;
}

// Re-export PointProjection from types so consumers can import from here
...
export function getTileSkewVectors(transform: ISymbolTransform): {vecEast: vec2; vecSouth: vec2} {
 ...
 */
export function updateLineLabels(bucket: SymbolBucketLike,
    painter: PainterLike,
    isText: boolean,
    ...
    unwrappedTileID: UnwrappedTileIDLike,
...
export function placeFirstAndLastGlyph(
    ...
    const firstGlyphOffset = (glyphOffsetArray as any).getoffsetX(symbol.glyphStartIndex);
    const lastGlyphOffset = (glyphOffsetArray as any).getoffsetX(glyphEndIndex - 1);
...
function placeGlyphsAlongLine(args: GlyphLinePlacementArgs): GlyphLinePlacementResult {
    ...
    if (symbol.numGlyphs > 1) {
        ...
        for (let glyphIndex = symbol.glyphStartIndex + 1; glyphIndex < glyphEndIndex - 1; glyphIndex++) {
            ...
            const placedGlyph = placeGlyphAlongLine(fontScale * (glyphOffsetArray as any).getoffsetX(glyphIndex), lineOffsetX, lineOffsetY, flip, symbol.segment,
                lineStartIndex, lineEndIndex, projectionContext, rotateToLine);
            if (!placedGlyph) {
                return {notEnoughRoom: true};
            }
        ...
        if (keepUpright && !flip) {
            ...
            const tileSegmentEnd = new Point((projectionContext.lineVertexArray as any).getx(tileVertexIndex), (projectionContext.lineVertexArray as any).gety(tileVertexIndex));
        ...
        const singleGlyph = placeGlyphAlongLine(fontScale * (glyphOffsetArray as any).getoffsetX(symbol.glyphStartIndex), lineOffsetX, lineOffsetY, flip, symbol.segment,
...
export function projectLineVertexToLabelPlane(index: number, projectionContext: SymbolProjectionContext, syntheticVertexArgs: ProjectionSyntheticVertexArgs): Point {
    ...
    const currentVertex = new Point(
        (projectionContext.lineVertexArray as any).getx(index),
        (projectionContext.lineVertexArray as any).gety(index));
    ...
    const previousTilePoint = syntheticVertexArgs.distanceFromAnchor === 0 ?
        ...
        new Point((projectionContext.lineVertexArray as any).getx(previousLineVertexIndex), (projectionContext.lineVertexArray as any).gety(previousLineVertexIndex));
...
export function hideGlyphs(num: number, dynamicLayoutVertexArray: StructArray<string>) {
    for (let i = 0; i < num; i++) {
 ...
 */
// STUB: globe projection — this function is primarily used for non-mercator projections
// where the pitchedLabelPlaneMatrix is insufficient. For mercator, the label plane matrix
// is accurate and this reprojection path is not needed.
 ...
 * Does not modify the input array.
 */
// STUB: globe projection — occluded points only occur with globe/vertical-perspective projections
// where geometry can be hidden behind the planet surface. For mercator, no points are ever occluded.
