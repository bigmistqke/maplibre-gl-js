...
// Source: src/symbol/collision_index.ts
...
function isCounterClockwise(a: Point, b: Point, c: Point): boolean {
    return (c.y - a.y) * (b.x - a.x) > (b.y - a.y) * (c.x - a.x);
}

// ---------------------------------------------------------------------------
// Inlined from maplibre-gl-js src/util/intersection_tests.ts
// Only polygonIntersectsPolygon and its dependencies are needed here.
// ---------------------------------------------------------------------------

function lineSegmentIntersectsLineSegment(a0: Point, a1: Point, b0: Point, b1: Point): boolean {
    return isCounterClockwise(a0, b0, b1) !== isCounterClockwise(a1, b0, b1) &&
        isCounterClockwise(a0, a1, b0) !== isCounterClockwise(a0, a1, b1);
}

function lineIntersectsLine(lineA: Array<Point>, lineB: Array<Point>): boolean {
...
function polygonContainsPoint(ring: Array<Point>, p: Point): boolean {
    let c = false;
...
function polygonIntersectsPolygon(polygonA: Array<Point>, polygonB: Array<Point>): boolean {
...
// ---------------------------------------------------------------------------
// Inlined minimal Bounds (from maplibre-gl-js src/geo/bounds.ts)
// ---------------------------------------------------------------------------
...
class Bounds {
    ...
    extend(point: {x: number; y: number}): this {
...
// ---------------------------------------------------------------------------
// ONE_EM inlined from maplibre-gl-js src/symbol/one_em.ts
// ---------------------------------------------------------------------------

const ONE_EM = 24;

/** Minimal SingleCollisionBox (matches MapLibre's type). */
...
/** Minimal OverscaledTileID interface (only fields used by CollisionIndex). */
export type OverscaledTileIDLike = {
    overscaledZ: number;
    wrap?: number;
};

// ---------------------------------------------------------------------------
// Public types exported by CollisionIndex
// ---------------------------------------------------------------------------
...
export class CollisionIndex {
    ...
    placeCollisionCircles(
        ...
        translation: [number, number],
        getElevation: (x: number, y: number) => number  // STUB: terrain elevation — pass () => 0 for flat maps
    ): PlacedCircles {
        const placedCollisionCircles: Array<number> = [];
        ...
        const projectionCache = {projections: {}, offsets: {}, cachedAnchorPoint: undefined as Point | undefined, anyProjectionOccluded: false};
    ...
    projectAndGetPerspectiveRatio(
        ...
        if (simpleProjectionMatrix) {
            ...
            if (getElevation) { // slow because of handle z-index
                // STUB: terrain elevation — passes getElevation through to elevation lookup
                pos = [x, y, getElevation(x, y), 1] as unknown as vec4;
                vec4.transformMat4(pos, pos, simpleProjectionMatrix);
            } else { // fast because of ignore z-index
                pos = [x, y, 0, 1] as unknown as vec4;
            ...
            // STUB: terrain elevation — passes getElevation through; use () => 0 for flat terrain
            const projected = this.transform.projectTileCoordinates(x, y, unwrappedTileID, getElevation ?? (() => 0));
            return {
    ...
    getPerspectiveRatio(x: number, y: number, unwrappedTileID: UnwrappedTileIDLike, getElevation?: (x: number, y: number) => number): number {
