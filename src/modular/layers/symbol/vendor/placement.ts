...
// Source: src/symbol/placement.ts
// Modifications: import paths adjusted, types narrowed, terrain STUB'd,
...
/** Minimal SymbolInstance — matches StructArray get(i) return shape */
interface SymbolInstance {
    crossTileID: number;
    textBoxScale: number;
    textAnchorOffsetStartIndex: number;
    textAnchorOffsetEndIndex: number;
    useRuntimeCollisionCircles: boolean;
    ...
    numVerticalGlyphVerticesIcon?: number;
}

interface TextAnchorOffset {
    textAnchor: number;
    textOffset0: number;
    textOffset1: number;
}

interface CollisionVertexArray {
    emplaceBack(...args: number[]): void;
    clear(): void;
}

interface CollisionBoxArray {
    // opaque — passed through only
    [key: string]: any;
}
...
interface UnwrappedTileID {
    canonical: { z: number; x: number; y: number };
    wrap: number;
}

/** Minimal SymbolBucket interface */
interface SymbolBucket {
    ...
    layers: Array<{
        ...
        layout: any;
        paint: any;
        _unevaluatedLayout?: any;
    }>;
    symbolInstances: {
        length: number;
        get(i: number): SymbolInstance;
    };
    textSizeData: any;
    sortKeyRanges: Array<{ sortKey: number; symbolInstanceStart: number; symbolInstanceEnd: number }>;
    collisionArrays: CollisionArrays[];
    text: {
        placedSymbolArray: { get(i: number): any };
        dynamicLayoutVertexArray: any;
        opacityVertexArray: any;
        opacityVertexBuffer?: { updateData(a: any): void };
        layoutVertexArray: { length: number };
    ...
    textCollisionBox: {
        ...
        collisionVertexBuffer?: { updateData(a: any): void };
    };
    iconCollisionBox: {
        collisionVertexArray: CollisionVertexArray;
        collisionVertexBuffer?: { updateData(a: any): void };
    ...
    textAnchorOffsets: { get(i: number): TextAnchorOffset };
    ...
    deserializeCollisionBoxes(collisionBoxArray: any): void;
...
interface Tile {
    ...
    getBucket(layer: any): any;
}

/** Minimal StyleLayer interface */
interface StyleLayer {
    id: string;
}

/** Minimal FeatureIndex interface */
interface FeatureIndex {
    [key: string]: any;
}

/** Extended transform for placement — adds getFastPathSimpleProjectionMatrix */
interface PlacementTransform extends ISymbolTransform {
    getFastPathSimpleProjectionMatrix?(tileID: OverscaledTileID): mat4 | undefined;
}

// ---------------------------------------------------------------------------
// Inlined from symbol_layout.ts — getAnchorJustification
// ---------------------------------------------------------------------------

function getAnchorJustification(anchor: TextAnchor): TextJustify {
    switch (anchor) {
        case 'right':
        case 'top-right':
        case 'bottom-right':
...
// ---------------------------------------------------------------------------
// Original placement.ts code (algorithm unchanged)
// ---------------------------------------------------------------------------
...
export class Placement {
    ...
    // STUB: terrain — always null, terrain-specific code guarded
    terrain: null;
    ...
    constructor(transform: PlacementTransform, _terrain: null | undefined, fadeDuration: number, crossSourceCollisions: boolean, prevPlacement?: Placement) {
        // NOTE: original calls transform.clone() — our TransformAdapter doesn't have clone(),
        // so we store the reference directly. This is safe as long as the transform isn't
        // mutated between placement and commit.
        this.transform = transform;
        this.terrain = null; // STUB: terrain
    ...
    private _getTerrainElevationFunc(_tileID: OverscaledTileID) {
        // STUB: terrain — always returns null
        return null;
    }
    ...
    getBucketParts(results: Array<BucketPart>, styleLayer: StyleLayer, tile: Tile, sortAcrossTiles: boolean) {
        ...
        const textPixelRatio = tile.tileSize / TILE_EXTENT;
    ...
    placeLayerBucketPart(bucketPart: BucketPart, seenCrossTileIDs: {
        ...
        const placeSymbol = (symbolInstance: SymbolInstance, collisionArrays: CollisionArrays, symbolIndex: number) => {
            ...
            let placedVerticalText: PlacedBox = {box: null, placeable: false, offscreen: null, occluded: false};
            ...
            if (textBox) {
                ...
                const placeTextForPlacementModes = (placeHorizontalFn: () => PlacedBox, placeVerticalFn: () => PlacedBox) => {
                ...
                if (textAnchorOffsetEnd === textAnchorOffsetStart) {
                    ...
                    const placeVertical = () => {
                        ...
                        return {box: null, offscreen: null} as PlacedBox;
                    };
                    ...
                    let prevAnchor = TextAnchorEnum[this.prevPlacement?.variableOffsets[symbolInstance.crossTileID]?.anchor] as number | undefined;
                    ...
                    const placeVertical = () => {
                        ...
                        return {box: null, occluded: true, offscreen: null} as PlacedBox;
                    };
    ...
    updateBucketOpacities(bucket: SymbolBucket, tileID: OverscaledTileID, seenCrossTileIDs: {
        ...
        const addOpacities = (iconOrText: any, numVertices: number, opacity: number) => {
                ...
                if (collisionArrays) {
                    ...
                    if (collisionArrays.textBox || collisionArrays.verticalTextBox) {
                        ...
                        if (collisionArrays.textBox || collisionArrays.verticalTextBox) {
                            ...
                            updateCollisionVertices(bucket.textCollisionBox.collisionVertexArray as any, opacityState.text.placed, !used || hidden, realBoxes.text, shift.x, shift.y);
                        }
                    }
                    ...
                    if (collisionArrays.iconBox || collisionArrays.verticalIconBox) {
                        ...
                        updateCollisionVertices(bucket.iconCollisionBox.collisionVertexArray as any, opacityState.icon.placed, hidden, realBoxes.icon,
