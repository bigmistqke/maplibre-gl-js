...
type SymbolTileRenderState = {
    ...
    terrainData: TerrainData | undefined | null;
    state: {
        program: Program<any>;
        buffers: SymbolBuffers;
        uniformValues: UniformValues<SymbolSDFUniformsType> | UniformValues<SymbolIconUniformsType>;
        ...
        program: Program<any>;
        buffers: SymbolBuffers;
        uniformValues: UniformValues<SymbolIconUniformsType>;
        projectionData: ProjectionData;
        atlasTexture: Texture;
        atlasTextureIcon: Texture | null;
        atlasInterpolation: TextureFilter;
...
export function drawSymbols(painter: Painter, tileManager: TileManager, layer: SymbolStyleLayer, coords: Array<OverscaledTileID>, variableOffsets: {
    ...
    if (assertedNotNullish(tileManager.map).showCollisionBoxes) {
...
function updateVariableAnchors(coords: Array<OverscaledTileID>,
    ...
    rotationAlignment: NonNullable<SymbolLayerSpecification['layout']>['text-rotation-alignment'],
...
function drawLayerSymbols(
    ...
    if (sortFeaturesByKey) {
        // @ts-expect-error - UNEXPECTED BEHAVIOR: sortKey can be undefined, undefined - undefined = NaN
    ...
    for (const segmentState of tileRenderState) {
        ...
        if (state.isSDF) {
            ...
            if (state.hasHalo) {
                // @ts-expect-error UNEXPECTED BEHAVIOR: uniformValues can be both UniformValues<SymbolSDFUniformsType> | UniformValues<SymbolIconUniformsType>
                //                                       see drawLayerSymbols
