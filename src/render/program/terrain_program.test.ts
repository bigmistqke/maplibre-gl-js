...
describe('terrainUniformValues', () => {
    test('disables fog when in globe projection mode', () => {
        ...
        const skyProperties = assertedNotNullish(sky.properties);
        ...
        expect(uniformValues['u_fog_color']).toEqual(skyProperties.get('fog-color'));
        expect(uniformValues['u_fog_ground_blend']).toBe(skyProperties.get('fog-ground-blend'));
    });
