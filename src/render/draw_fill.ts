...
function drawFillTiles(
    ...
    for (const coord of coords) {
        ...
        if (!isOutline) {
            ...
            uniformValues = image ? fillPatternUniformValues(painter, assertedNotNullish(crossfade), tile, translateForUniforms) : fillUniformValues(translateForUniforms);
        } else {
            indexBuffer = bucket.indexBuffer2;
            segments = bucket.segments2;
            const drawingBufferSize = [gl.drawingBufferWidth, gl.drawingBufferHeight] as [number, number];
            uniformValues = (programName === 'fillOutlinePattern' && image) ?
                fillOutlinePatternUniformValues(painter, assertedNotNullish(crossfade), tile, drawingBufferSize, translateForUniforms) :
        ...
        program.draw(painter.context, drawMode, depthMode,
            ...
            layer.id, assertedNotNullish(bucket.layoutVertexBuffer), indexBuffer, segments,
