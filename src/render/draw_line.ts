...
export function drawLine(painter: Painter, sourceCache: SourceCache, layer: LineStyleLayer, coords: Array<OverscaledTileID>, renderOptions: RenderOptions) {
    ...
    for (const coord of coords) {
        ...
        const constantPattern = patternProperty.constantOr(null as any);
        const constantDasharray = dasharrayProperty && dasharrayProperty.constantOr(null as any);
        ...
        if (constantPattern && tile.imageAtlas) {
            ...
            if (dashTo && dashFrom) programConfiguration.setConstantDashPositions(dashTo, dashFrom);
        }
        ...
        if (image) {
            ...
            bindGradientAndDashTextures(painter, sourceCache, context, gl, layer, bucket, coord, programConfiguration, assertedNotNullish(crossfade));
        } else if (dasharray) {
