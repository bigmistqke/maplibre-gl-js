...
export class ProjectionErrorMeasurement {
    ...
    public destroy() {
        ...
        assertedNotNullish(this._fullscreenTriangle).destroy();
        assertedNotNullish(this._fbo).destroy();
    ...
    private _bindFramebuffer() {
        ...
        const gl = context.gl;
        context.activeTexture.set(gl.TEXTURE1);
        gl.bindTexture(gl.TEXTURE_2D, assertedNotNullish(this._fbo).colorAttachment.get());
        context.bindFramebuffer.set(assertedNotNullish(this._fbo).framebuffer);
    }
    ...
    private _renderErrorTexture(input: number, outputExpected: number): void {
        ...
        program.draw(context, gl.TRIANGLES,
            ...
            '$clipping', assertedNotNullish(this._fullscreenTriangle).vertexBuffer, assertedNotNullish(this._fullscreenTriangle).indexBuffer,
            assertedNotNullish(this._fullscreenTriangle).segments);
