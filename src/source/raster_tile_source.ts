...
export class RasterTileSource extends Evented implements Source {
    ...
    async load(sourceDataChanged: boolean = false) {
        ...
        try {
            const tileJSON = await loadTileJson(this._options, assertedNotNullish(this.map)._requestManager, this._tileJSONRequest, assertedNotNullish(this.map)._ownerWindow);
