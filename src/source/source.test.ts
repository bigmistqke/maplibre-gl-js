...
describe('addSourceType', () => {
    test('adds factory function without a worker url does not dispatch to worker', async () => {
        const sourceType = vi.fn().mockImplementation(function (this: any, id: string) { this.id = id; }) as unknown as SourceClass;
        ...
        create('id', {type: 'foo'} as any, null as any, null as any);
    ...
    test('create a custom source without an id throws', async () => {
        ...
        expect(() => create('id', {type: 'foo2'} as any, null as any, null as any)).toThrow();
        expect(sourceType).toHaveBeenCalled();
    });
