...
export class Style extends Evented {
    ...
    _glyphsDidChange: boolean = false;
    _updatedPaintProps: {[layer: string]: true} = {};
    _layerOrderChanged: boolean = false;
    ...
    private _createLayers() {
        ...
        for (const layer of dereferencedLayers) {
            ...
            assertNotNullish(styledLayer.source);

            if (isRasterStyleLayer(styledLayer) && this.tileManagers[styledLayer.source]) {
                // @ts-expect-error - accessing raster-fade-duration on a narrowed raster layer paint union
                const rasterFadeDuration = layer.paint?.['raster-fade-duration'] ?? assertedNotNullish(styledLayer.paint).get('raster-fade-duration');
    ...
    _validateLayer(layer: StyleLayer) {
        ...
        const tileManager = this.tileManagers[layerSource];
        if (!tileManager) {
            return;
        }
    ...
    update(parameters: EvaluationParameters) {
        ...
        if (changed) {
            ...
            for (const id in this._updatedPaintProps) {
                assertedNotNullish(this._layers)[id].updateTransitions(parameters);
            }
        ...
        const managersUsedBefore: Record<string, boolean | undefined> = {};
        ...
        for (const layerId of this._order) {
            const layer = assertedNotNullish(this._layers)[layerId];

            layer.recalculate(parameters, this._availableImages);
            if (!layer.isHidden(parameters.zoom) && layer.source) {
                assertedNotNullish(this.tileManagers)[layer.source].used = true;
            }
        }
    ...
    addSource(id: string, source: SourceSpecification | CanvasSourceSpecification, options: StyleSetterOptions = {}) {
        this._checkLoaded();
        assertNotNullish(this.tileManagers);
    ...
    _updatePaintProperty(layer: StyleLayer, name: string, value: any, options: StyleSetterOptions = {}) {
        ...
        if (isRasterStyleLayer(layer) && name === 'raster-fade-duration') {
            this.tileManagers[assertedNotNullish(layer.source)].setRasterFadeDuration(value);
        }
    ...
    _updatePlacement(transform: ITransform, showCollisionBoxes: boolean, fadeDuration: number, crossSourceCollisions: boolean, forceFullPlacement: boolean = false) {
        ...
        for (const layerID of this._order) {
            ...
            assertNotNullish(styleLayer.source);
            ...
            if (!layerTiles[styleLayer.source]) {
                ...
                layerTiles[styleLayer.source] = tileManager.getRenderableIds(true)
                    .map((id) => assertedNotNullish(tileManager.getTileByID(id)))
