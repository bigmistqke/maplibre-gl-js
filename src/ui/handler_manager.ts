...
class RenderFrameEvent extends Event {
    // @ts-expect-error maybe this needs a different pattern
    //                  e.g. initialize property via `override readonly type = 'renderFrame'` instead of depending on setting it in constructor
...
export class HandlerManager {
    ...
    constructor(map: Map, options: CompleteMapOptions) {
        ...
        for (const [target, type, listenerOptions] of this._listeners) {
            const handler = (assertedNotNullish(target) === this._ownerDocument ? this.handleWindowEvent : this.handleEvent) as EventListener;
